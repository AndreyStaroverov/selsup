...
public class CrptApi {
    ...
    public synchronized void createDocument(Document document, String signature) throws IOException, InterruptedException {
        ...
        long checkTime = currentTime - lastRequest;
        long timeInterval = timeUnit.toMillis(1);

        if (checkTime < timeInterval && requestCount >= requestLimit) {
            long sleepTime = timeInterval - checkTime;
        ...
        if (currentTime - lastRequest >= timeInterval) {
            requestCount = 1;
        } else {
            requestCount++;
        }
